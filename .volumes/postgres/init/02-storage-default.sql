...
CREATE SCHEMA IF NOT EXISTS storage AUTHORIZATION vietlach_admin;
...
CREATE USER storage_admin NOINHERIT CREATEROLE LOGIN NOREPLICATION;
GRANT ALL PRIVILEGES ON SCHEMA storage TO storage_admin;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA storage TO storage_admin;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA storage TO storage_admin;
ALTER USER storage_admin SET search_path = "storage";
ALTER table "storage".objects owner to storage_admin;
ALTER table "storage".buckets owner to storage_admin;
ALTER table "storage".migrations OWNER TO storage_admin;
ALTER function "storage".foldername(text) owner to storage_admin;
ALTER function "storage".filename(text) owner to storage_admin;
ALTER function "storage".extension(text) owner to storage_admin;
ALTER function "storage".search(text,text,int,int,int) owner to storage_admin;
